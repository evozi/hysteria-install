...
# updated by https://github.com/evozi/hysteria-install
...
get_version() {
    ...
    TMP_FILE="$(mktemp)"
    #if ! curl -x "${PROXY}" -sS -i -H "Accept: application/vnd.github.v3+json" -o "$TMP_FILE" 'https://api.github.com/repos/apernet/hysteria/releases/latest'; then
    #    "rm" "$TMP_FILE"
    ...
    cat << EOF >> "${JSON_PATH}/config.json"
{
    ...
    "acme": {
        ...
        "email": "evozi@gmail.com"
    },
    "obfs": "i am not a ninja",
    "up_mbps": 100,
    "down_mbps": 100
}
EOF
